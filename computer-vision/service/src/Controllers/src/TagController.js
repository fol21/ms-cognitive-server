...
class SpecificController {
     ...
     */
    init(router) {
        router.post('/tag',upload.single('file'),bodyParser.json(),this.fileMiddle, this.base64Middle, this.analyzeMiddle, this.sendJson);
    ...
    analyzeMiddle(req, res, next) {
        res.analyzePromise = msCVApi.tag(res.content);
        next(); // pass to next middleware
    }
